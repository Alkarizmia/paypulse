        ...
        await Promise.all(
          recipients.map((recipientId) =>
            createNotificationServer(supabase, {
              ...
              body: `${winner.client.name} — ${winner.client.due_date}`,
              payload: {
                clientId: winner.client.id,
                dueDate: winner.client.due_date,
        ...
        recipients.map((recipientId) =>
          Promise.all([
            ...
            createNotificationServer(supabase, {
              ...
              workspaceId: job.workspace_id,
              type: "reminder_stage",
              title: "reminder_stage",
              body: `${client.name} — J+${job.schedule_days}`,
              payload: {
                jobId: job.id,
                clientId: client.id,
                stage: `J+${job.schedule_days}`,
                scheduleDays: job.schedule_days,
                clientName: client.name,
                email: client.email,
              },
