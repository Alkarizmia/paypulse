  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <div
          className="pp-rise pp-dashboard-card-interactive rounded-2xl border border-white/[0.08] bg-[#14141c] p-5 shadow-[0_0_0_1px_rgba(139,92,246,0.06)] hover:border-violet-500/25"
        ...
        >
          ...
          <p className="mt-3 text-2xl font-semibold tabular-nums tracking-tight text-white">{money.format(pendingAmount)}</p>
          <p className="mt-1 text-xs font-medium text-slate-500">{t.instant}</p>
        </div>
        <div
          className="pp-rise pp-dashboard-card-interactive rounded-2xl border border-white/[0.08] bg-[#14141c] p-5 hover:border-emerald-500/20"
        ...
        >
          ...
          <p className="mt-3 text-2xl font-semibold tabular-nums tracking-tight text-white">{money.format(receivedThisMonth)}</p>
        ...
        <div
          className="pp-rise pp-dashboard-card-interactive rounded-2xl border border-white/[0.08] bg-[#14141c] p-5 hover:border-orange-500/25"
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="rounded-lg bg-orange-500/15 p-2 text-orange-400">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </span>
          </div>
          <p className="mt-3 text-2xl font-semibold tabular-nums tracking-tight text-white">{overdueCount}</p>
          <p className="mt-1 text-xs font-medium text-slate-500">{t.instant}</p>
        </div>
      </div>
      ...
      {advancedStats ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="pp-dashboard-card-interactive rounded-2xl border border-violet-500/20 bg-violet-500/[0.06] px-5 py-4 hover:border-violet-400/35">
          ...
          <div className="pp-dashboard-card-interactive rounded-2xl border border-violet-500/20 bg-violet-500/[0.06] px-5 py-4 hover:border-violet-400/35">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{paidPct}%</p>
          </div>
        </div>
      ) : null}
      ...
      {fullCharts ? (
        <div className="grid gap-6 lg:grid-cols-5">
          <div className="pp-dashboard-card-interactive rounded-2xl border border-white/[0.08] bg-[#14141c] p-5 lg:col-span-3 hover:border-violet-500/20">
            ...
            <div className="mt-6 h-44">
              ...
              <div className="mt-1 flex justify-between px-1 text-[10px] font-medium uppercase tracking-wider text-slate-500">
                {monthLabels.map((m) => (
                  <span key={m}>{m}</span>
                ))}
              </div>
          ...
          <div className="pp-dashboard-card-interactive rounded-2xl border border-white/[0.08] bg-[#14141c] p-5 lg:col-span-2 hover:border-violet-500/20">
                ...
                >
                  <div className="flex h-full w-full items-center justify-center rounded-full bg-[#14141c]">
                    <div className="text-center">
                      ...
                      <span className="text-sm font-bold tabular-nums text-white">{money.format(totalVolume)}</span>
                    </div>
                  </div>
                </div>
              </div>
