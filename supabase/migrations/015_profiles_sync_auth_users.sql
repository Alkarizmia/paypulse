...
-- ---------------------------------------------------------------------------
-- Rattrapage : tous les profils existants
-- ---------------------------------------------------------------------------
-- ⚠️ Ne lance PAS seulement ce bloc DO sans le CREATE FUNCTION ci-dessus :
--    sinon erreur « function public.sync_auth_user_from_profile(uuid) does not exist ».
--    Exécute ce fichier ENTIER, ou utilise `supabase/manual_profiles_sync_auth_complete.sql`.
do $$
declare
  uid uuid;
begin
